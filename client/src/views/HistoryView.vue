        ...
        <v-card elevation="0" class="pa-2 pt-2 kanban-card-pro fullpage-card flat-card kanban-inner-padding" style="position: relative; height: calc(100vh - 70px - 4vw); min-height: calc(100vh - 70px - 4vw); width: calc(100vw - 4vw); min-width: calc(100vw - 4vw); border-radius: 0; box-shadow: none; margin: 0 auto;">
          ...
          <template>
            <div class="kanban-hero">
              ...
              <div class="kanban-hero-title">History</div>
              ...
              <v-btn
                ...
                >
                  <v-icon left size="18" >mdi-cloud-print</v-icon>
                  Print
                </v-btn>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    totalRows() {
      ...
      const batchDataSet = this.selectedBatchId 
        ...
        : null;
      const rowCount = Number(batchDataSet) || Number(this.dataSet) || 5;
      return Math.max(5, rowCount);
    },
    maxRowPage() {
      const rows = this.totalRows;
      return Math.max(0, Math.ceil(rows / this.rowsPerPage) - 1);
    },
    visibleRowIndices() {
      const rows = this.totalRows;
      const start = this.rowPage * this.rowsPerPage;
      const end = Math.min(start + this.rowsPerPage, rows);
    ...
    rowRangeLabel() {
      ...
      const end = Math.min(start + (this.rowsPerPage - 1), rows);
  ...
  async mounted() {
    ...
    watch: {
      async selectedBatchId(val){
        this.kanbanDateHistoryList = []
        const selectedBatch = this.batchList.find(b => b.id === val);
        ...
        if(this.kanbanDateHistoryList.length > 0){
      ...
      async selectedDate(val){
        ...
        await this.fetchKanbanHistory(val , selectedBatch.name );
      //  const selectBatchDate = this.kanbanDateHistoryList.find(rec=> rec.id == val)
      //  this.fetchKanbanHistory(this.selectedBatchId, this.selectedDate);
      },
  ...
  methods: {
        ...
        async generateQRCodeImage(payload) {
        ...
        async generateBarcodeImage(payload) {
          if (!payload) return null;
        ...
        async mapKanbanHistoryToBoard(historyItems) {
          ...
          // Helper mapping from possible header tokens to column names
          const headerMap = {
            'FCRH': 'FSC RH',
            'F/CRH': 'FSC RH',
            'FSCRH': 'FSC RH',
          ...
          for (const item of historyItems) {
            ...
            const columnIndex = this.currentData.columns.findIndex(c => c.header === foundHeader);
            if (columnIndex === -1) continue;

            const rowIndex = this.findFirstEmptySlot(columnIndex);
            if (rowIndex === -1 || rowIndex >= this.totalRows) continue;

            const column = this.currentData.columns[columnIndex];

            // Generate visuals and place item
            const qrCode = await this.generateQRCodeImage(raw);
            const scannedBarcodeImg = item.barcode ? await this.generateBarcodeImage(item.barcode) : null;

            this.$set(column.items, rowIndex, {
              qrData: raw,
              headerName: foundHeader,
              qrCode,
            ...
            if (raw && !this.scannedList.includes(raw)) this.scannedList.push(raw);
            if (item.barcode && !this.scannedList.includes(item.barcode)) this.scannedList.push(item.barcode);
          }
        ...
        findFirstEmptySlot(columnIndex) {
          ...
          for (let i = 0; i < this.totalRows; i++) {
            const item = items[i];
            if (!item || Object.keys(item).length === 0 || !item.qrData) {
              return i;
            }
          }
          return -1; 
    ...
    async getModel(){
        ...
        async fetchKanbanHistory(set,kanban) {
          try {
            this.isGenerating = true;
            const selectedSet = set || this.selectedBatchId;
            const selectedKanban = kanban || this.selectedDate;
            if (!selectedSet) {
              return;
            }

            const response = await api.get('/stationtwo/kanban/history', {
              params: {
                set: selectedSet,
            ...
            this.kanbanHistory = response.data || [];

            await this.mapKanbanHistoryToBoard(this.kanbanHistory);
            this.rowPage = 0;
          } catch (error) {
            console.error('Error fetching kanban history:', error);
            this.saveErrorLog('fetchKanbanHistory', error);
          } finally {
    ...
    detectScanType(value) {
      if (value.length < 13) return "Barcode"; // Quick heuristic for long values
      if(value.length >= 14) return "QR"; // Too short to determine
...
.batch-select:hover{border-color:rgba(45, 49, 52, 0.3);box-shadow:0 4px 16px rgba(25,118,210,0.15);}
.batch-select >>> .v-input__control{min-height:38px;}.batch-select >>> .v-input__slot{padding:0 12px !important;}
.batch-select >>> .v-select__selection{font-weight:700;font-size:14px;color:#050606;}
.date-select{width:250px;height:40px;padding:0 12px;border-radius:8px;background:linear-gradient(135deg,#ffffff,#f8fafc);border:1px solid rgba(25,118,210,0.15);box-shadow:0 2px 8px rgba(25,118,210,0.08);font-weight:700;letter-spacing:0.3px;color:#0d0d0d;}
.date-select:hover{border-color:rgba(25,118,210,0.3);box-shadow:0 4px 16px rgba(25,118,210,0.15);}
.date-select:focus{outline:none;box-shadow:0 0 0 4px rgba(25,118,210,0.08);}
.date-input-wrapper{display:flex;flex-direction:column;justify-content:flex-start;gap:4px;}
.date-input-label{font-size:12px;font-weight:700;color:#1976d2;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:2px;}
