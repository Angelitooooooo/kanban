...
const getKanbanQAByDateAndKanban = async (req, res) => {
	try {
		...
		const results = await db('Kanban_qa')
			...
			.andWhere('kanban', 'like', `%${kanban}%`)
			.where(function() {
				this.where('isValidatedQR', 1).orWhere('isValidatedBarcode', 1);
			})
			.orderBy('id', 'asc');
            // console.log('getKanbanQAByDateAndKanban - Query results:', results);

		// Map results to requested structure
		const formatted = results.map(item => ({
			columnName: item.kanban ? item.kanban.startsWith('RSC')
            ? item.kanban.slice(0,3)
            : item.kanban.slice(0,6) : '',
			row:  parseInt(item.qr_kanban?.match(/\d+$/)?.[0] || 0, 10),
